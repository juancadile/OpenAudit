...
class AnalysisModuleRegistry:
    ...
    def register_module(
        self, 
        name: str, 
        module_class: Type[BaseAnalysisModule],
        override: bool = False
    ) -> bool:
        """
        ...
        if name in self.modules and not override:
            logger.warning(f"Module '{name}' already registered. Use override=True to replace.")
            return False
        
        # Validate module class
        if not issubclass(module_class, BaseAnalysisModule):
            logger.error(f"Module class {module_class} does not inherit from BaseAnalysisModule")
            return False
        ...
        try:
            ...
            if not instance.is_valid:
                logger.warning(f"Module '{name}' has dependency issues but will be registered")
    ...
    def get_modules_by_category(self, category: ModuleCategory) -> List[str]:
        ...
        return [
            name for name, instance in self.module_instances.items()
    ...
    def validate_module_compatibility(self, modules: List[str]) -> Dict[str, Any]:
        ...
        if invalid_modules:
            result["issues"].extend([f"Module has dependency issues: {m}" for m in invalid_modules])
    ...
    def create_analysis_pipeline(self, selected_modules: List[str]) -> Dict[str, Any]:
        ...
        pipeline = {
            ...
            "estimated_requirements": self._calculate_pipeline_requirements(selected_modules)
        }
        ...
        for name in selected_modules:
            ...
            if instance:
                ...
                raise ModuleCompatibilityError(f"Failed to create instance of module: {name}")
    ...
    def _calculate_pipeline_requirements(self, modules: List[str]) -> Dict[str, Any]:
        ...
        instances = [self.module_instances[name] for name in modules if name in self.module_instances]
        
        if not instances:
            return {}
        
        # Calculate maximum requirements
        max_samples = max(inst.module_info.requirements.min_samples for inst in instances)
...
def register_module(name: str, module_class: Type[BaseAnalysisModule], override: bool = False) -> bool:
