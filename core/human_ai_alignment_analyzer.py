...
class HumanAIAlignmentModule(BaseAnalysisModule):
    ...
    def analyze(self, data: Any, **kwargs) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "recommendations": ["Check data format and ensure paired ratings are provided"],
    ...
    def _extract_ratings(self, data: Any, **kwargs) -> Tuple[List[float], List[float]]:
        ...
        if isinstance(data, dict):
            if 'human_ratings' in data and 'ai_ratings' in data:
                ...
                raise ValueError("Dictionary must contain 'human_ratings' and 'ai_ratings' keys")
            ...
            for item in data:
                if isinstance(item, dict):
                    if 'human' in item and 'ai' in item:
                        ...
                        raise ValueError("Each item must contain human and AI rating keys")
                else:
                    raise ValueError("List items must be dictionaries with paired ratings")
        ...
        # Check kwargs for ratings
            ...
            raise ValueError("Unable to extract human and AI ratings from provided data")
    ...
    def _create_enhanced_alignment_analysis(
        ...
        **kwargs
        ...
        # Additional analysis
        enhanced_analysis = self._perform_additional_alignment_analysis(human_ratings, ai_ratings)
        
        # Extract key findings
        key_findings = self._extract_alignment_findings(alignment_results, enhanced_analysis)
        
        # Generate recommendations
        recommendations = self._generate_alignment_recommendations(alignment_results, enhanced_analysis)
        
        # Calculate confidence score
        confidence_score = self._calculate_alignment_confidence(alignment_results, len(human_ratings))
        ...
        summary = {
            ...
            "concerning_misalignment": alignment_results.get('concerning_misalignment', False),
    ...
    def _perform_additional_alignment_analysis(
        ...
        midpoint = (np.max(np.concatenate([human_array, ai_array])) + 
                   np.min(np.concatenate([human_array, ai_array]))) / 2
        
        human_direction = human_array - midpoint
        ai_direction = ai_array - midpoint
        directional_agreement = np.sum(np.sign(human_direction) == np.sign(ai_direction))
        ...
        consistency_ratio = min(human_std, ai_std) / max(human_std, ai_std) if max(human_std, ai_std) > 0 else 1.0
        ...
        range_similarity = min(human_range, ai_range) / max(human_range, ai_range) if max(human_range, ai_range) > 0 else 1.0
    ...
    def validate_input(self, data: Any) -> Dict[str, Any]:
        ...
        try:
            ...
            if len(human_ratings) != len(ai_ratings):
                validation["errors"].append("Human and AI ratings must have the same length")
                validation["valid"] = False
            
            if len(human_ratings) < 3:
                validation["warnings"].append("Very few rating pairs - results may be unreliable")
            ...
            human_range = max(human_ratings) - min(human_ratings) if human_ratings else 0
    ...
    def _extract_alignment_findings(
        ...
        if alignment_strength == 'strong':
            findings.append(f"Strong human-AI alignment detected (ρ = {correlation:.3f})")
        ...
        if agreement_rate >= 0.8:
            findings.append(f"High agreement rate: {agreement_rate:.1%} of ratings within tolerance")
        elif agreement_rate >= 0.6:
            findings.append(f"Moderate agreement rate: {agreement_rate:.1%} of ratings within tolerance")
        else:
            findings.append(f"Low agreement rate: {agreement_rate:.1%} of ratings within tolerance")
        ...
        if consistency_ratio < 0.5:
            findings.append("Large difference in rating consistency between humans and AI")
    ...
    def _generate_alignment_recommendations(
        ...
        if concerning_misalignment or alignment_strength == 'very_weak':
            recommendations.append("🚨 CONCERNING MISALIGNMENT: Significant discrepancy between human and AI judgments")
            recommendations.append("Review AI decision-making process and consider retraining")
        elif alignment_strength == 'weak':
            recommendations.append("⚠️ WEAK ALIGNMENT: AI judgments differ from human assessments")
            recommendations.append("Consider calibration training to improve alignment")
        elif alignment_strength == 'moderate':
            recommendations.append("ℹ️ MODERATE ALIGNMENT: Some differences in judgment detected")
            recommendations.append("Monitor for patterns in disagreements")
        else:
            recommendations.append("✅ STRONG ALIGNMENT: AI judgments align well with human assessments")
        ...
        if agreement_rate < 0.6:
            recommendations.append("Low agreement rate suggests need for improved calibration")
        
        # Extreme disagreement recommendations
        extreme_rate = enhanced_analysis.get('extreme_disagreement_rate', 0)
        if extreme_rate > 0.2:
            recommendations.append("Investigate cases of extreme disagreement for insights")
    ...
    def _calculate_alignment_confidence(self, alignment_results: Dict[str, Any], sample_size: int) -> float:
