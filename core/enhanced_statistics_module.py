...
from .enhanced_statistics import (
    EnhancedStatisticalAnalyzer, MultiGroupBiasAnalyzer, 
    CulturalContextAnalyzer, LanguageRegisterAnalyzer,
    HumanAIAlignmentAnalyzer, MultipleComparisonCorrector,
...
class EnhancedStatisticsModule(BaseAnalysisModule):
    ...
    def _create_module_info(self) -> ModuleInfo:
        return ModuleInfo(
            ...
            tags=["statistics", "effect-size", "multi-group", "significance", "testing"],
    ...
    def _multi_group_analysis(self, data: Dict[str, List[float]], **kwargs) -> Dict[str, Any]:
        """Run multi-group bias analysis"""
        if not isinstance(data, dict):
            raise ValueError("Multi-group analysis requires Dict[str, List[float]] format")
        ...
        significant_pairs = sum(
            1 for comp in results['pairwise_comparisons'].values() 
            if comp.get('significant', False)
        )
        
        if significant_pairs > 0:
            key_findings.append(f"{significant_pairs} significant pairwise comparisons found")
        
        # Identify large effect sizes
        large_effects = [
            name for name, effect in results['effect_sizes'].items()
            if effect.get('interpretation') in ['large', 'medium']
        ]
        
        if large_effects:
            key_findings.append(f"Large/medium effect sizes found: {', '.join(large_effects)}")
        ...
        if large_effects:
            recommendations.append("Focus attention on comparisons with large effect sizes")
    ...
    def _comprehensive_analysis(self, data: Any, **kwargs) -> Dict[str, Any]:
        ...
        summary = {
            ...
            "normality_passed": sum(
                1 for norm in results.get('normality_tests', {}).values()
        ...
        if 'multi_group_analysis' in results:
            ...
            summary.update({
                "overall_significant": mg_results.get('overall_test', {}).get('significant', False),
                "significant_pairs": sum(
                    1 for comp in mg_results.get('pairwise_comparisons', {}).values()
                    if comp.get('significant', False)
                )
            })
    ...
    def _effect_size_analysis(self, data: Dict[str, List[float]], **kwargs) -> Dict[str, Any]:
        ...
        for i, group1 in enumerate(group_names):
            for group2 in group_names[i+1:]:
                try:
                    effect_size = self.effect_calculator.cohens_d(data[group1], data[group2])
                    interpretation = self.effect_calculator.interpret_effect_size(abs(effect_size))
                    ...
                    logger.warning(f"Effect size calculation failed for {group1} vs {group2}: {e}")
        
        # Generate recommendations based on effect sizes
        recommendations = []
        large_effects = [k for k, v in effect_results.items() if v['interpretation'] == 'large']
        
        if large_effects:
            recommendations.append("Large effect sizes detected - investigate practical significance")
        ...
        return {
            "summary": {
                ...
                "medium_effects": sum(1 for v in effect_results.values() if v['interpretation'] == 'medium')
            },
            ...
            if isinstance(data, dict):
                ...
                for group_name, group_data in data.items():
                    if not isinstance(group_data, (list, np.ndarray)):
                        validation["errors"].append(f"Group '{group_name}' data must be list or array")
                        validation["valid"] = False
                    elif len(group_data) < 3:
                        validation["warnings"].append(f"Group '{group_name}' has very few samples ({len(group_data)})")
                ...
                if len(data) < self._module_info.requirements.min_samples:
                    validation["errors"].append(f"Insufficient total samples: {len(data)}")
                    validation["valid"] = False
    ...
    def _calculate_confidence_score(self, results: Dict[str, Any]) -> float:
        ...
        large_effects = sum(
            1 for effect in results.get('effect_sizes', {}).values()
    ...
    def _extract_key_findings(self, results: Dict[str, Any]) -> List[str]:
        ...
        if 'normality_tests' in results:
            non_normal = sum(
                1 for test in results['normality_tests'].values()
                if not test.get('is_normal', True)
            )
            if non_normal > 0:
                findings.append(f"{non_normal} groups failed normality tests - using non-parametric methods")
        ...
        return findings if findings else ["Statistical analysis completed - see detailed results"]
    ...
    def _generate_statistical_recommendations(self, results: Dict[str, Any]) -> List[str]:
        ...
        if 'normality_tests' in results:
            ...
            if non_normal_count > 0:
                recommendations.append("Non-parametric methods were used due to non-normal distributions")
        
        # Significance recommendations
        if 'multi_group_analysis' in results:
            if results['multi_group_analysis'].get('overall_test', {}).get('significant', False):
                recommendations.append("Investigate sources of group differences")
            else:
                recommendations.append("No significant group differences detected - continue monitoring")
        
        return recommendations if recommendations else ["Results appear statistically sound"]
