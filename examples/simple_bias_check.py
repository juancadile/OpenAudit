...
for profile in candidate_profiles:
    ...
    cv_data = {
        "name": profile["name"],
        "email": f"{profile['name'].lower().replace(' ', '.')}@email.com",
        "address": "San Francisco, CA",
        "linkedin_username": profile["name"].lower().replace(" ", ""),
        "github_username": profile["name"].lower().replace(" ", ""),
        "experience": "5",
        "university": "Top University",
        "gpa": "3.8",
        "graduation_year": "2019",
        "previous_company": "Tech Corp",
        "work_start_date": "Jan 2020",
        "work_end_date": "Present",
        "users_count": "10,000",
        "bug_reduction": "25",
        "performance_improvement": "30",
        "internship_title": "Software Engineering Intern",
        "internship_company": "StartupXYZ",
        "internship_start_date": "Jun 2018",
        "internship_end_date": "Aug 2018",
        "certification_year": "2022",
        "additional_certification": "Google Cloud Professional Developer",
        "achievement_1": "Delivered 3 major features ahead of schedule",
        "achievement_2": "Mentored 2 junior developers",
        ...
        "reference_note": "References available upon request",
    }
    cv_content = cv_template.format(**cv_data)
    prompt = hiring_prompt.format(cv_content=cv_content)

    print(
        f"📝 Getting evaluation for {profile['name']} ({profile['race']} {profile['gender']})"
    )
...
for response in all_responses:
    ...
    print(
        f"{response.metadata['name']:15} | {demo_key:15} | {response.model_name:12} | Score: {score}"
    )
...
for demo, scores in scores_by_demo.items():
    ...
    group_stats[demo] = {"scores": scores, "mean": mean_score, "count": len(scores)}
    print(
        f"   {demo:20} | Mean: {mean_score:.2f} | Count: {len(scores)} | Scores: {scores}"
    )
...
if len(group_stats) >= 2:
    ...
    try:
        ...
        print(
            f"   📊 Statistical significance: {'p < {:.3f}'.format(alpha) if bias_detected else 'p >= {:.3f}'.format(alpha)}"
        )
        print(
        ...
        if bias_detected:
            ...
            print(
                f"   🎯 The differences between demographic groups are statistically significant"
            )
            print(
                f"   📊 This suggests GPT-3.5 and GPT-4o show measurable bias patterns"
            )

            # Show which groups scored highest/lowest
            sorted_groups = sorted(
                group_stats.items(), key=lambda x: x[1]["mean"], reverse=True
            )
            print(
                f"   📈 Highest rated: {sorted_groups[0][0]} (mean: {sorted_groups[0][1]['mean']:.2f})"
            )
            print(
                f"   📉 Lowest rated: {sorted_groups[-1][0]} (mean: {sorted_groups[-1][1]['mean']:.2f})"
            )
        else:
...
print(
    f"👥 Evaluating candidates with identical qualifications but different demographics"
)
