...
class MultiLLMDispatcher:
    ...
    def _setup_models(self):
        ...
        if os.getenv("OPENAI_API_KEY"):
            ...
            # GPT-4 series
            self.models["gpt-4o"] = ChatOpenAI(model="gpt-4o", temperature=0, seed=42)
            ...
            self.models["gpt-4-turbo"] = ChatOpenAI(
                model="gpt-4-turbo", temperature=0, seed=42
            )

            # GPT-4.1 series (latest generation)
            try:
                self.models["gpt-4.1-nano"] = ChatOpenAI(
                    model="gpt-4.1-nano", temperature=0, seed=42
                )
            ...
            try:
                self.models["gpt-4.1-mini"] = ChatOpenAI(
                    model="gpt-4.1-mini", temperature=0, seed=42
                )
            ...
            try:
                self.models["gpt-4.1"] = ChatOpenAI(
                    model="gpt-4.1", temperature=0, seed=42
                )
                print("✓ Added GPT-4.1")
            except Exception as e:
    ...
    async def dispatch_prompt(
        ...
        if not available_models:
            raise ValueError(
                f"No available models found. Check API keys and model names."
            )

        print(
            f"Dispatching to {len(available_models)} models with {iterations} iterations each..."
        )
        ...
        for model_name, model in available_models.items():
            for i in range(iterations):
                tasks.append(
                    self._query_model(model, model_name, prompt, i, temperature)
                )
...
async def main():
    ...
    for prompt in prompts:
        ...
        responses = await dispatcher.dispatch_prompt(
            prompt=prompt, iterations=3  # Test each prompt 3 times per model
        )
