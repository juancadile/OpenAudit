...
try:
    ...
    print("⚠️  Anthropic not available - install langchain-anthropic for Claude support")
...
try:
    ...
    print("⚠️  Google Gemini not available - install langchain-google-genai for support")
...
class ModelManager:
    ...
    def _initialize_models(self):
        ...
        print(f"✅ Initialized {len(self.models)} models across {len(set(c.provider for c in self.model_configs.values()))} providers")
    ...
    def _setup_openai_models(self):
        ...
        openai_models = [
            ...
            ModelConfig("gpt-3.5-turbo", ModelProvider.OPENAI, "gpt-3.5-turbo", 
                       "GPT-3.5 Turbo", "fast", cost_per_1k_tokens=0.002,
                       description="Fast, cost-effective model for most tasks"),
            
            # GPT-4 Series
            ModelConfig("gpt-4", ModelProvider.OPENAI, "gpt-4", 
                       "GPT-4", "general", cost_per_1k_tokens=0.06,
                       description="Most capable GPT-4 model"),
            ModelConfig("gpt-4-turbo", ModelProvider.OPENAI, "gpt-4-turbo", 
                       "GPT-4 Turbo", "general", cost_per_1k_tokens=0.03,
                       description="Faster GPT-4 with updated knowledge"),
            ModelConfig("gpt-4o", ModelProvider.OPENAI, "gpt-4o", 
                       "GPT-4o", "general", cost_per_1k_tokens=0.015,
                       description="Optimized GPT-4 for speed and cost"),
            ModelConfig("gpt-4o-mini", ModelProvider.OPENAI, "gpt-4o-mini", 
                       "GPT-4o Mini", "fast", cost_per_1k_tokens=0.0075,
                       description="Compact GPT-4o for high-volume tasks"),
            
            # GPT-4.1 Series (if available)
            ModelConfig("gpt-4.1-nano", ModelProvider.OPENAI, "gpt-4.1-nano", 
                       "GPT-4.1 Nano", "fast", cost_per_1k_tokens=0.005,
                       description="Ultra-fast GPT-4.1 variant"),
            ModelConfig("gpt-4.1-mini", ModelProvider.OPENAI, "gpt-4.1-mini", 
                       "GPT-4.1 Mini", "general", cost_per_1k_tokens=0.01,
                       description="Balanced GPT-4.1 model"),
            ModelConfig("gpt-4.1", ModelProvider.OPENAI, "gpt-4.1", 
                       "GPT-4.1", "general", cost_per_1k_tokens=0.03,
                       description="Latest GPT-4.1 full model"),
            
            # Reasoning Models (o1 Series)
            ModelConfig("o1-preview", ModelProvider.OPENAI, "o1-preview", 
                       "o1 Preview", "reasoning", supports_temperature=False,
                       cost_per_1k_tokens=0.15,
                       description="Advanced reasoning model - preview version"),
            ModelConfig("o1-mini", ModelProvider.OPENAI, "o1-mini", 
                       "o1 Mini", "reasoning", supports_temperature=False,
                       cost_per_1k_tokens=0.12,
                       description="Compact reasoning model"),
            ModelConfig("o1", ModelProvider.OPENAI, "o1", 
                       "o1", "reasoning", supports_temperature=False,
                       cost_per_1k_tokens=0.20,
                       description="Full o1 reasoning model"),
            
            # o3 Series (newest reasoning)
            ModelConfig("o3-mini", ModelProvider.OPENAI, "o3-mini", 
                       "o3 Mini", "reasoning", supports_temperature=False,
                       cost_per_1k_tokens=0.15,
                       description="Latest compact reasoning model"),
            ModelConfig("o3", ModelProvider.OPENAI, "o3", 
                       "o3", "reasoning", supports_temperature=False,
    ...
    def _setup_anthropic_models(self):
        ...
        claude_models = [
            ModelConfig("claude-3-5-sonnet", ModelProvider.ANTHROPIC, "claude-3-5-sonnet-20241022",
                       "Claude 3.5 Sonnet", "general", cost_per_1k_tokens=0.015,
                       description="Most capable Claude model"),
            ModelConfig("claude-3-5-haiku", ModelProvider.ANTHROPIC, "claude-3-5-haiku-20241022",
                       "Claude 3.5 Haiku", "fast", cost_per_1k_tokens=0.008,
                       description="Fast and efficient Claude model"),
            ModelConfig("claude-3-opus", ModelProvider.ANTHROPIC, "claude-3-opus-20240229",
                       "Claude 3 Opus", "general", cost_per_1k_tokens=0.075,
                       description="Most powerful Claude 3 model"),
            ModelConfig("claude-3-sonnet", ModelProvider.ANTHROPIC, "claude-3-sonnet-20240229",
                       "Claude 3 Sonnet", "general", cost_per_1k_tokens=0.015,
                       description="Balanced Claude 3 model"),
            ModelConfig("claude-3-haiku", ModelProvider.ANTHROPIC, "claude-3-haiku-20240307",
                       "Claude 3 Haiku", "fast", cost_per_1k_tokens=0.0025,
    ...
    def _setup_xai_models(self):
        ...
        grok_models = [
            ModelConfig("grok-beta", ModelProvider.XAI, "grok-beta",
                       "Grok Beta", "general", cost_per_1k_tokens=0.05,
                       description="xAI's Grok model - witty and capable"),
            ModelConfig("grok-vision-beta", ModelProvider.XAI, "grok-vision-beta",
                       "Grok Vision Beta", "general", cost_per_1k_tokens=0.05,
    ...
    def _setup_google_models(self):
        ...
        gemini_models = [
            ModelConfig("gemini-pro", ModelProvider.GOOGLE, "gemini-pro",
                       "Gemini Pro", "general", cost_per_1k_tokens=0.0005,
                       description="Google's most capable model"),
            ModelConfig("gemini-pro-vision", ModelProvider.GOOGLE, "gemini-pro-vision",
                       "Gemini Pro Vision", "general", cost_per_1k_tokens=0.0025,
                       description="Gemini with vision capabilities"),
            ModelConfig("gemini-1.5-pro", ModelProvider.GOOGLE, "gemini-1.5-pro",
                       "Gemini 1.5 Pro", "general", cost_per_1k_tokens=0.0035,
                       description="Latest Gemini Pro model"),
            ModelConfig("gemini-1.5-flash", ModelProvider.GOOGLE, "gemini-1.5-flash",
                       "Gemini 1.5 Flash", "fast", cost_per_1k_tokens=0.00075,
    ...
    def _setup_custom_models(self):
        ...
        for model_name, endpoint in custom_endpoints.items():
            if endpoint:
                try:
                    config = ModelConfig(
                        model_name, ModelProvider.CUSTOM, model_name,
                        model_name.replace("-", " ").title(), "custom",
                        description=f"Custom model via {endpoint}"
                    )
                    
                    model = ChatOpenAI(
                        base_url=endpoint,
                        api_key=os.getenv(f"{model_name.upper().replace('-', '_')}_API_KEY", "dummy"),
                        model=model_name,
                        temperature=0
                    )
    ...
    async def query_model(self, model_name: str, prompt: str, temperature: float = 0.0) -> ModelResponse:
