...
from core.module_registry import get_global_registry, register_module, clear_global_registry
from core.analysis_profiles import get_global_profile_manager, AnalysisProfile
...
Examples:
  ...
  %(prog)s test quick                     # Run quick tests
        ...
        start_parser = subparsers.add_parser('start', help='Start OpenAudit web interface')
        start_parser.add_argument('--port', '-p', type=int, default=5100,
                                 help='Port to run web interface on')
        start_parser.add_argument('--debug', action='store_true',
                                 help='Run in debug mode')
        
        # Module management commands
        modules_parser = subparsers.add_parser('modules', help='Manage analysis modules')
        ...
        # modules info
        info_parser = modules_subparsers.add_parser('info', help='Get module information')
        info_parser.add_argument('module_name', help='Name of module to get info about')
        
        # modules register (for external modules)
        register_parser = modules_subparsers.add_parser('register', help='Register an external module')
        register_parser.add_argument('name', help='Module name')
        register_parser.add_argument('path', help='Path to module file')
        register_parser.add_argument('--class-name', help='Module class name (if different from filename)')
        register_parser.add_argument('--no-security-check', action='store_true',
                                   help='Skip security validation (use with caution)')
        
        # modules create (template)
        create_parser = modules_subparsers.add_parser('create', help='Create external module template')
        ...
        # modules load-all
        modules_subparsers.add_parser('load-all', help='Load all external modules from directory')
        
        # modules list-external
        modules_subparsers.add_parser('list-external', help='List loaded external modules')
        
        # modules test
        test_parser = modules_subparsers.add_parser('test', help='Test module compatibility')
        test_parser.add_argument('module_names', nargs='+', help='Module names to test')
        
        # Profile management commands
        profiles_parser = subparsers.add_parser('profiles', help='Manage analysis profiles')
        ...
        # profiles show
        show_parser = profiles_subparsers.add_parser('show', help='Show profile details')
        show_parser.add_argument('profile_name', help='Name of profile to show')
        
        # profiles create
        create_parser = profiles_subparsers.add_parser('create', help='Create new profile')
        create_parser.add_argument('name', help='Profile name')
        create_parser.add_argument('--description', '-d', help='Profile description')
        create_parser.add_argument('--modules', '-m', nargs='+', required=True,
                                  help='List of modules to include')
        create_parser.add_argument('--params', '-p', help='JSON string of default parameters')
        
        # profiles validate
        validate_parser = profiles_subparsers.add_parser('validate', help='Validate profile')
        ...
        analyze_parser.add_argument('--modules', '-m', nargs='+', help='Specific modules to use')
        ...
        analyze_parser.add_argument('--format', choices=['json', 'text'], default='text',
                                   help='Output format')
        analyze_parser.add_argument('--alpha', type=float, default=0.05,
                                   help='Significance level (default: 0.05)')
        analyze_parser.add_argument('--effect-size-threshold', type=float, default=0.1,
                                   help='Effect size threshold (default: 0.1)')
        
        # Test commands
        test_parser = subparsers.add_parser('test', help='Run test suite')
        test_parser.add_argument('suite', choices=['quick', 'modular', 'all'], default='quick',
    ...
    def cmd_modules_create(self, args) -> int:
        ...
        if result["success"]:
            ...
            print(f"2. Test with: python cli.py modules register {module_name} {result['module_file']}")
        else:
    ...
    def cmd_status(self, args) -> int:
        ...
        try:
            import requests
            response = requests.get('http://localhost:5100', timeout=2)
    ...
    def cmd_version(self, args) -> int:
        ...
        print("Features: Modular Analysis System, Enhanced Statistics, Cultural Context")
        print("License: MIT")
        return 0
