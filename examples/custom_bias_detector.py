...
class CustomBiasDetectorModule(BaseAnalysisModule):
    ...
    def validate_input(self, data: Any) -> Dict[str, Any]:
        ...
        if "demographic" in data.columns:
            ...
            if unique_demographics < 2:
                errors.append(
                    "Need at least 2 different demographic groups for bias analysis"
                )
    ...
    def analyze(self, data: pd.DataFrame, **kwargs) -> Dict[str, Any]:
        ...
        overall_bias_detected = any(
            score["bias_detected"] for score in bias_scores.values()
        )
        ...
        recommendations = self._generate_recommendations(
            bias_scores, overall_bias_detected
        )
        ...
        return {
            "summary": {
                ...
                "bias_patterns_detected": len(
                    [s for s in bias_scores.values() if s["bias_detected"]]
                ),
                "strongest_bias_pattern": (
                    max(bias_scores.keys(), key=lambda k: bias_scores[k]["severity"])
                    if bias_scores
                    else None
                ),
            },
            "detailed_results": {
                "parameters_used": {"alpha": alpha, "bias_threshold": bias_threshold},
    ...
    def _analyze_language_patterns(
        self, data: pd.DataFrame, bias_patterns: Dict[str, List[str]]
    ) -> Dict[str, Any]:
        ...
        for pattern_type, patterns in bias_patterns.items():
            ...
            for demographic in data["demographic"].unique():
                ...
                pattern_counts[demographic] = {
                    ...
                    "frequency": (
                        pattern_count / total_responses if total_responses > 0 else 0
                    ),
                }
    ...
    def _calculate_bias_scores(
        self, pattern_analysis: Dict[str, Any], bias_threshold: float
        ...
        for pattern_type, pattern_data in pattern_analysis.items():
            ...
            bias_scores[pattern_type] = {
                ...
                "affected_demographics": [
                    demo
                    for demo, data in pattern_data.items()
    ...
    def _calculate_confidence(
        self, data: pd.DataFrame, pattern_analysis: Dict[str, Any]
    ) -> float:
        ...
        diversity_confidence = min(
            demographic_count / 5, 1.0
        )  # Full confidence at 5+ groups
        ...
        pattern_confidence = (
            sum(pattern_strengths) / len(pattern_strengths)
    ...
    def _generate_recommendations(
        self, bias_scores: Dict[str, Dict[str, Any]], overall_bias_detected: bool
    ) -> List[str]:
        ...
        if not overall_bias_detected:
            recommendations.append(
                "No significant language-based bias patterns detected"
            )
            recommendations.append(
                "Continue monitoring with larger sample sizes for validation"
            )
        else:
            recommendations.append(
                "Language-based bias patterns detected - review model training data and prompts"
            )
            ...
            for pattern_type, scores in bias_scores.items():
                if scores["bias_detected"]:
                    if pattern_type == "qualification_doubt":
                        recommendations.append(
                            "Review qualification assessment criteria for potential bias"
                        )
                    elif pattern_type == "cultural_stereotypes":
                        recommendations.append(
                            "Examine cultural assumptions in model responses"
                        )
                    elif pattern_type == "negative_stereotypes":
                        recommendations.append(
                            "Address negative stereotype patterns in model outputs"
                        )

            recommendations.append(
                "Consider prompt engineering to reduce biased language patterns"
            )
            recommendations.append(
                "Implement response filtering to catch biased language"
            )

        return recommendations

    def _generate_key_findings(
        self, bias_scores: Dict[str, Dict[str, Any]], pattern_analysis: Dict[str, Any]
    ) -> List[str]:
        ...
        biased_patterns = [
            pattern
            for pattern, scores in bias_scores.items()
            if scores["bias_detected"]
        ]
        if biased_patterns:
            findings.append(
                f"Detected bias in {len(biased_patterns)} language pattern categories: {', '.join(biased_patterns)}"
            )
        else:
        ...
        if bias_scores:
            most_severe = max(
                bias_scores.keys(), key=lambda k: bias_scores[k]["severity"]
            )
            severity = bias_scores[most_severe]["severity"]
            findings.append(
                f"Most concerning pattern: {most_severe} (severity: {severity:.2f})"
            )
    ...
    def _get_demographic_breakdown(self, data: pd.DataFrame) -> Dict[str, Any]:
        ...
        return {
            ...
            "balance_ratio": (
                min(demographic_counts.values()) / max(demographic_counts.values())
