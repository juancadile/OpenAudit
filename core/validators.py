...
def validate_university_name(university: str) -> bool:
    ...
    # Allow letters (including accented), spaces, numbers, hyphens, apostrophes, and common punctuation
    pattern = r"^[\w\s\-'.,&()àáâäèéêëìíîïòóôöùúûüÀÁÂÄÈÉÊËÌÍÎÏÒÓÔÖÙÚÛÜçÇñÑ]+$"
    ...
    if not isinstance(prompt_template, str) or not prompt_template.strip():
        raise ValidationError(
            ...
            details={
                ...
                "type": type(prompt_template).__name__,
            },
        )
...
def validate_model_response(response: Dict[str, Any]) -> None:
    ...
    # Validate model name
    if (
        not isinstance(response["model_name"], str)
        or not response["model_name"].strip()
    ):
        raise ValidationError(
...
def validate_file_path(file_path: Union[str, Path], must_exist: bool = True) -> Path:
    ...
    if must_exist and not path.exists():
        raise ValidationError(
            f"File does not exist: {path}", details={"file_path": str(path)}
        )

    if must_exist and not path.is_file():
        raise ValidationError(
            f"Path is not a file: {path}", details={"file_path": str(path)}
        )
...
def validate_json_structure(data: Any, expected_structure: Dict[str, type]) -> None:
    ...
    if not isinstance(data, dict):
        raise ValidationError(
            "Data must be a dictionary", details={"data_type": type(data).__name__}
        )
...
def validate_module_interface(module_class: Type) -> Dict[str, Any]:
    """
    Validate that a module class properly implements the BaseAnalysisModule interface.

    Args:
        module_class: Class to validate
    ...
    try:
        ...
        errors.append("Cannot import BaseAnalysisModule for validation")
        return {"valid": False, "errors": errors, "warnings": warnings}

    # Check if it inherits from BaseAnalysisModule
    if not issubclass(module_class, BaseAnalysisModule):
        errors.append(
            f"Class {module_class.__name__} must inherit from BaseAnalysisModule"
        )
        return {"valid": False, "errors": errors, "warnings": warnings}

    # Check required methods
    required_methods = ["analyze", "_create_module_info", "get_supported_data_types"]
    ...
    # Check if analyze method has correct signature
    if hasattr(module_class, "analyze"):
        try:
            sig = inspect.signature(module_class.analyze)
            params = list(sig.parameters.keys())

            # Should have at least 'self' and 'data' parameters
            if "data" not in params:
                errors.append("analyze() method must have 'data' parameter")
            ...
            if not has_kwargs:
                warnings.append(
                    "analyze() method should accept **kwargs for flexibility"
                )

        except Exception as e:
            warnings.append(f"Could not inspect analyze() method signature: {str(e)}")

    # Try to create an instance and validate module info
    try:
        instance = module_class()

        # Check if module_info property exists and is valid
        if hasattr(instance, "module_info"):
            ...
            for field in required_info_fields:
                if not hasattr(module_info, field) or not getattr(module_info, field):
                    warnings.append(f"Module info missing or empty field: {field}")
        else:
            errors.append("Module must have module_info property")

    except Exception as e:
        warnings.append(f"Could not create instance for validation: {str(e)}")
