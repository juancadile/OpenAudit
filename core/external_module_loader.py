...
from .module_registry import get_global_registry, AnalysisModuleRegistry
...
class ModuleSecurityValidator:
    ...
    SAFE_IMPORTS = {
        ...
        'typing', 'dataclasses', 'datetime', 'json', 'pathlib', 'logging',
        'collections', 'itertools', 'functools', 'statistics', 'math'
    }
            ...
            if line.startswith('import ') or line.startswith('from '):
                ...
                if module in self.RESTRICTED_IMPORTS:
                    if self.strict_mode:
                        ...
                        self.warnings.append(f"Line {i}: Potentially unsafe import '{module}'")
                elif module not in self.SAFE_IMPORTS and not module.startswith('core.'):
                    self.warnings.append(f"Line {i}: Unknown import '{module}' - verify safety")
    ...
    def _check_dangerous_functions(self, source_code: str):
        ...
        for i, line in enumerate(lines, 1):
            for func in dangerous_functions:
                if func in line:
                    self.errors.append(f"Line {i}: Dangerous function call '{func.rstrip('(')}'")
    ...
    def _check_file_operations(self, source_code: str):
        ...
        for i, line in enumerate(lines, 1):
            for op in file_ops:
                if op in line:
                    if self.strict_mode:
                        self.errors.append(f"Line {i}: File operation '{op.rstrip('(')}' not allowed")
                    else:
                        self.warnings.append(f"Line {i}: File operation '{op.rstrip('(')}' detected")
    ...
    def _check_network_operations(self, source_code: str):
        ...
        for i, line in enumerate(lines, 1):
            for op in network_ops:
                if op in line:
                    self.errors.append(f"Line {i}: Network operation '{op.rstrip('.')}' not allowed")
...
class ExternalModuleLoader:
    ...
    def load_module_from_file(self, 
        ...
        if not module_path.suffix == '.py':
            return {"success": False, "error": "Module file must be a Python (.py) file"}
        ...
        try:
            ...
            if validate_security:
                security_result = self.security_validator.validate_source_code(source_code)
            ...
            if auto_register:
                ...
                if success:
                    self.loaded_modules[module_name]["loaded_at"] = module_class.__name__
                    # Save manifest
            ...
            return {
                ...
                "security_warnings": security_result.get("warnings", []) if validate_security else []
            }
    ...
    def load_module_from_manifest(self, manifest_path: Union[str, Path]) -> Dict[str, Any]:
    ...
    def load_all_modules(self, directory: Optional[Union[str, Path]] = None) -> Dict[str, Any]:
    ...
    def create_module_template(self, 
                             module_name: str,
                             author: str = "Unknown",
                             description: str = "Custom analysis module") -> Dict[str, Any]:
...
'''
    ...
    def load_module_from_github(self, 
                               github_url: str,
                               module_name: Optional[str] = None,
                               branch: str = "main",
    ...
    def install_module_from_github(self,
                                  github_url: str,
                                  module_name: Optional[str] = None,
                                  **kwargs) -> Dict[str, Any]:
        """
    ...
    def _download_github_repo(self, owner: str, repo: str, branch: str = "main") -> Dict[str, Any]:
    ...
    def _file_contains_analysis_module(self, file_path: Path) -> bool:
        ...
        try:
            ...
            return (
                'BaseAnalysisModule' in content and 
                ('class ' in content) and
                ('def analyze(' in content or 'def analyze(self' in content)
            )
        except Exception:
            return False
    
    def _find_analysis_module_class(self, module, class_name: Optional[str] = None) -> Optional[Type]:
        ...
        for name, obj in inspect.getmembers(module, inspect.isclass):
            ...
            if (issubclass(obj, BaseAnalysisModule) and 
                obj != BaseAnalysisModule and
    ...
    def _create_manifest(self, 
                        ...
                        module_path: Path, 
                        class_name: str,
                        source_code: str) -> ModuleManifest:
...
def load_external_module(module_path: Union[str, Path], 
                        module_name: Optional[str] = None,
                        **kwargs) -> Dict[str, Any]:
