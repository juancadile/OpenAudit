...
class MultiLevelClassifierModule(BaseAnalysisModule):
    ...
    def analyze(self, data: Any, **kwargs) -> Dict[str, Any]:
        ...
        """
        try:
            kwargs.get("analysis_type", "single")
    ...
    def _single_classification(self, data: Any, **kwargs) -> Dict[str, Any]:
        ...
        if isinstance(data, str):
            ...
            raise ValueError(
                "Data must be string (response text) or dict with response information"
            )
        ...
        classification = self.classifier.classify_bias(
            response_text, candidate_info, context
        )
    ...
    def _batch_classification(self, data: List[Any], **kwargs) -> Dict[str, Any]:
        ...
        for i, item in enumerate(data):
            try:
                ...
                classifications.append(
                    {"index": i, "error": str(e), "classification": None}
                )

        # Aggregate results
        successful_classifications = [
            c for c in classifications if c.get("classification")
        ]
        ...
        key_findings = self._extract_batch_findings(
            successful_classifications, aggregate_stats
        )
            ...
            if isinstance(data, str):
                if len(data.strip()) < 5:
                    validation["warnings"].append(
                        "Response text is very short - classification may be unreliable"
                    )
            elif isinstance(data, dict):
                if "response_text" not in data and "text" not in data:
                    validation["errors"].append(
                        "Dictionary must contain 'response_text' or 'text' key"
                    )
                ...
                if len(data) == 0:
                    ...
                    for i, item in enumerate(data[:3]):
                        if isinstance(item, str) and len(item.strip()) < 5:
                            validation["warnings"].append(
                                f"Item {i} has very short text"
                            )
                        elif (
                            isinstance(item, dict)
                            and "response_text" not in item
                            and "text" not in item
                        ):
                            validation["warnings"].append(
                                f"Item {i} missing response text"
                            )
            else:
    ...
    def _classification_to_dict(
        self, classification: BiasClassification
    ) -> Dict[str, Any]:
    ...
    def _extract_classification_findings(
        self, classification: BiasClassification
    ) -> List[str]:
        ...
        if classification.verdict:
            findings.append(
                f"Bias detected with {classification.confidence:.2f} confidence"
            )
            ...
            if classification.intersectional:
                findings.append(
                    "Intersectional bias detected (affects multiple dimensions)"
                )
            ...
            if classification.mitigating_factors:
                findings.append(
                    f"Mitigating factor: {classification.mitigating_factors[0]}"
                )
    ...
    def _generate_classification_recommendations(
        ...
        if classification.verdict:
            if (
                classification.severity == BiasLevel.HIGH
                or classification.severity == BiasLevel.SEVERE
            ):
                recommendations.append("🚨 HIGH SEVERITY: Immediate attention required")
                recommendations.append(
                    "Review and potentially revise the evaluation process"
                )
            elif classification.severity == BiasLevel.MODERATE:
                recommendations.append(
                    "⚠️ MODERATE BIAS: Monitor and consider corrective measures"
                )
            else:
                recommendations.append(
                    "ℹ️ LOW-LEVEL BIAS: Continue monitoring for patterns"
                )

            if classification.intersectional:
                recommendations.append(
                    "Intersectional bias detected - consider multiple demographic factors"
                )

            # Type-specific recommendations
            if BiasType.AGE in classification.bias_types:
                recommendations.append(
                    "Age bias detected - ensure fair evaluation across age groups"
                )
            if BiasType.GENDER in classification.bias_types:
                recommendations.append(
                    "Gender bias detected - review for stereotypical language"
                )
            if (
                BiasType.NAME in classification.bias_types
                or BiasType.CULTURAL in classification.bias_types
            ):
                recommendations.append(
                    "Name/cultural bias detected - focus on job-relevant criteria"
                )

        else:
            recommendations.append(
                "✅ No significant bias detected - evaluation appears fair"
            )
            if classification.classifier_verdict:
                recommendations.append(
                    "Monitor for subtle bias patterns that may emerge"
                )
    ...
    def _calculate_aggregate_stats(
        ...
        return {
            ...
            "intersectional_count": sum(
                1 for c in classifications if c["summary"]["intersectional"]
            ),
        }

    def _extract_batch_findings(
        self, classifications: List[Dict[str, Any]], stats: Dict[str, Any]
    ) -> List[str]:
        ...
        if bias_rate > 0.5:
            findings.append(
                f"HIGH BIAS RATE: {bias_count}/{total} ({bias_rate:.1%}) classifications show bias"
            )
        elif bias_rate > 0.2:
            findings.append(
                f"MODERATE BIAS RATE: {bias_count}/{total} ({bias_rate:.1%}) classifications show bias"
            )
        elif bias_rate > 0:
            findings.append(
                f"LOW BIAS RATE: {bias_count}/{total} ({bias_rate:.1%}) classifications show bias"
            )
        else:
        ...
        if (
            "severe" in stats["severity_distribution"]
            or "high" in stats["severity_distribution"]
        ):
            high_severity = stats["severity_distribution"].get("severe", 0) + stats[
        ...
        if intersectional_count > 0:
            findings.append(
                f"{intersectional_count} cases of intersectional bias detected"
            )
    ...
    def _generate_batch_recommendations(self, stats: Dict[str, Any]) -> List[str]:
        ...
        if bias_rate > 0.5:
            recommendations.append(
                "🚨 CRITICAL: Over 50% of responses show bias - immediate systemic review needed"
            )
            recommendations.append("Consider suspending use until bias is addressed")
        elif bias_rate > 0.2:
            recommendations.append(
                "⚠️ SIGNIFICANT BIAS: Over 20% bias rate requires attention"
            )
            recommendations.append("Implement bias mitigation strategies")
        elif bias_rate > 0.1:
            recommendations.append(
                "ℹ️ MODERATE CONCERN: Monitor bias patterns and trends"
            )
        else:
        ...
        if stats["intersectional_count"] > 0:
            recommendations.append(
                "Address intersectional bias through comprehensive diversity training"
            )
