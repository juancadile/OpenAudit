...
ANALYSIS_PROFILES = {
    ...
    "cultural": AnalysisProfile(
        ...
        default_parameters={
            ...
            "alpha": 0.01,
            "num_samples": 5,
            "contexts_to_test": ["diverse_team", "global_perspective", "local_community"]
        },
    ...
    "hiring_specific": AnalysisProfile(
        ...
        default_parameters={
            "contexts_to_test": [
                "diverse_team", "merit_only", "startup_culture", 
                "traditional_corporate", "age_inclusive", "gender_neutral"
            ],
    ...
    def get_profile_modules(self, profile_name: str, registry=None) -> List[str]:
        ...
        if "all" in modules:
            if registry:
                ...
                modules.remove("all")
                modules.extend([
                    "enhanced_statistics", "cultural_context", "multi_level_classifier",
                    "goal_conflict", "human_ai_alignment"
                ])
        
        return list(set(modules))  # Remove duplicates
    
    def validate_profile_compatibility(self, profile_name: str, registry=None) -> Dict[str, Any]:
