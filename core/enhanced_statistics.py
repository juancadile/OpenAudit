...
from scipy.stats import (
    kruskal, mannwhitneyu, wilcoxon, spearmanr, shapiro,
...
class MultiGroupBiasAnalyzer:
    ...
    def multi_group_bias_analysis(self, responses_by_group: Dict[str, List[float]]) -> Dict[str, Any]:
        """
        ...
        for i, group1 in enumerate(group_names):
            for j, group2 in enumerate(group_names[i+1:], i+1):
                try:
                    ...
                    # Effect size
                    n1, n2 = len(responses_by_group[group1]), len(responses_by_group[group2])
                    ...
                    logger.warning(f"Pairwise comparison failed for {group1} vs {group2}: {e}")
                    continue
    ...
    def enhanced_bias_test_with_effect_sizes(
        ...
        test_groups: Dict[str, List[float]]
    ) -> Dict[str, Any]:
        """
        ...
        for group_name, test_group in test_groups.items():
            try:
                ...
                if len(control_group) == len(test_group):
                    ...
                    u_stat, p_val = mannwhitneyu(control_group, test_group, alternative='two-sided')
                    test_type = 'mann_whitney_u'
                    r = self.effect_calculator.r_from_u(u_stat, len(control_group), len(test_group))
    ...
    async def cultural_context_analysis(
        ...
        for context, responses in contextualized_responses.items():
            try:
                ...
                direction = 'context_increases_bias' if mean_context > mean_base else 'context_decreases_bias'
    ...
    def language_register_bias_test(
        ...
        try:
            ...
            return {
                ...
                'effect_size': r,
                'interpretation': self.effect_calculator.interpret_effect_size(r),
                'direction': 'formal_favored' if mean_formal > mean_casual else 'casual_favored',
...
class EnhancedStatisticalAnalyzer:
    ...
    def check_normality(self, data: List[float]) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                'recommendation': 'use_parametric' if p_value > 0.05 else 'use_nonparametric'
            }
        except Exception as e:
    ...
    def comprehensive_bias_analysis(
        ...
        results = {
            ...
            'sample_sizes': {name: len(data) for name, data in responses_by_group.items()}
        }
        ...
        try:
            multi_group_results = self.multi_group_analyzer.multi_group_bias_analysis(responses_by_group)
        ...
        if control_group_name and control_group_name in responses_by_group:
            ...
            test_groups = {k: v for k, v in responses_by_group.items() if k != control_group_name}
            
            try:
                enhanced_results = self.multi_group_analyzer.enhanced_bias_test_with_effect_sizes(
        ...
        # Multiple comparison correction
        if 'multi_group_analysis' in results and 'pairwise_comparisons' in results['multi_group_analysis']:
            p_values = [
                comp['p_value'] 
                for comp in results['multi_group_analysis']['pairwise_comparisons'].values()
                if 'p_value' in comp
            ]
            
            if p_values:
                try:
                    correction_results = self.correction_handler.corrected_multiple_testing(
