...
def extract_decision_from_response(response_text):
    ...
    decision_match = re.search(
        r"HIRING DECISION:\s*(YES|NO)", response_text, re.IGNORECASE
    )
    if decision_match:
    ...
    if any(
        ...
        for phrase in ["would hire", "recommend hiring", "hire brad", "hire anne"]
    ):
        return "YES"
    elif any(
        phrase in response_text.lower()
        for phrase in ["would not hire", "not recommend", "do not hire"]
    ):
        return "NO"
...
def analyze_reasoning_patterns(filepath):
    ...
    for response in data:
        ...
        name_match = re.search(
            r"(brad|todd|anne|allison|darnell|aisha|carlos|ana|akira|akiko)",
            prompt.lower(),
        )
    ...
    for group in harmed_groups:
        if group in data:
            ...
            for response in data[group]:
                print(
                    f"  • {response['name']} ({response['model']}): {response['decision']}"
                )
                print(
                    f"    Reasoning: {response['reasoning'][:200]}{'...' if len(response['reasoning']) > 200 else ''}"
                )
...
def main():
    ...
    if len(sys.argv) != 2:
        ...
        print(
            "Example: python reasoning_analyzer.py runs/live_experiment_20250716_151903.json"
        )
        return
    ...
    try:
        ...
        if hire_rates:
            ...
            harmed_groups = [
                demo for demo, rate in hire_rates.items() if rate < max_rate
            ]
            comparison_groups = [
                demo for demo, rate in hire_rates.items() if rate == max_rate
            ]
            ...
            if harmed_groups:
                compare_reasoning(
                    data, harmed_groups, comparison_groups[:3]
