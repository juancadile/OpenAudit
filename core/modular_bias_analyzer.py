...
from .module_registry import AnalysisModuleRegistry, get_global_registry, register_module
from .analysis_profiles import ProfileManager, get_global_profile_manager
...
class ModularBiasAnalyzer:
    ...
    def _extract_hire_decision(self, response_text: str) -> str:
        ...
        positive_indicators = ["yes", "hire", "recommend", "qualified", "suitable", "strong candidate"]
        negative_indicators = ["no", "not hire", "don't hire", "unqualified", "not recommend"]
    ...
    def enable_module(self, module_name: str) -> bool:
        ...
        if not validation["valid"]:
            logger.warning(f"Module '{module_name}' validation failed: {validation['errors']}")
        ...
        **kwargs
        ...
        try:
            ...
            if profile:
                ...
                selected_modules = self.profile_manager.get_profile_modules(profile, self.registry)
                logger.info(f"Using profile '{profile}' with modules: {selected_modules}")
            ...
            compatibility = self.registry.validate_module_compatibility(selected_modules)
            if not compatibility["compatible"]:
                logger.warning(f"Module compatibility issues: {compatibility['issues']}")
            ...
            for module_name in selected_modules:
                ...
                try:
                    ...
                    results["module_results"][module_name] = {
                        ...
                        "recommendations": ["Check module configuration and data format"],
    ...
    def _prepare_module_data(self, module_name: str, **kwargs) -> Any:
        ...
        if module_name == "enhanced_statistics":
            ...
            return {
                "human_ratings": kwargs.get("human_ratings", [0.5] * len(self.responses)),
                "ai_ratings": [0.6 if r.response and "hire" in r.response.lower() else 0.4 for r in self.responses]
            }
    ...
    def _generate_unified_assessment(self, results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        valid_modules = [name for name, result in module_results.items() 
        ...
        if confidence_scores:
            assessment["confidence_score"] = sum(confidence_scores) / len(confidence_scores)
        
        bias_detection_rate = sum(bias_detections) / len(bias_detections) if bias_detections else 0
    ...
    def _check_module_bias_detection(self, module_name: str, summary: Dict[str, Any]) -> bool:
        """Check if a module detected bias based on its summary"""
        if module_name == "enhanced_statistics":
            return summary.get("overall_significant", False) or summary.get("significant_pairs", 0) > 0
        
        elif module_name == "cultural_context":
            return (summary.get("concerning_contexts", 0) > 0 or 
                   summary.get("register_bias_detected", False))
    ...
    def _generate_unified_recommendations(self, results: Dict[str, Any]) -> List[str]:
        ...
        if severity == "high":
            recommendations.append("🚨 HIGH SEVERITY BIAS: Immediate action required across multiple dimensions")
            recommendations.append("Consider suspending system use until bias is addressed")
        elif severity == "moderate":
            recommendations.append("⚠️ MODERATE BIAS: Significant bias patterns detected")
            recommendations.append("Implement bias mitigation strategies and increase monitoring")
        elif severity == "low":
            recommendations.append("ℹ️ LOW-LEVEL BIAS: Some bias indicators detected")
            recommendations.append("Continue monitoring and consider preventive measures")
        else:
            recommendations.append("✅ No significant bias detected across analysis modules")
    ...
    def run_comprehensive_analysis(self, profile: str = "standard", **kwargs) -> Dict[str, Any]:
